...
# Este arquivo "string.asm" contém a implementação das 5 funções requisitadas no trabalho.
# Dentre as funções temos: 

# Strlen -> recebe um ponteiro de uma string e retorna a quantidade de caracteres.

# strcmp -> Compara duas strings e retorna um inteiro. Se for negativo é porquê a primeira string é menor que a segunda
# se for positivo é porquê a primeira string é maior que a segunda e 0 é porquê elas são idênticas.

# strcat -> Concatena duas strings. A string src ao final da string dest.

# strncat -> contatena uma determinada quantidade da string src no final da string dest.

# strncpy -> Faz a mesma coisa que a strcpy, mas em vez de copiar todos os elementos, copia somente os count primeiros
# da src para a "dest".

.data

msgErro: .asciiz "\nErro: Você excedeu a quantidade de caracteres que a string possuí"

.text

strlen: # Função para contar os caracteres de uma string que está no registrador $a0, retorna o valor int em $v0.
addi $sp, $sp,-8 # Desce o ponteiro que aponta para pilha 3 vezes, ou seja, aloca 3 espaços na pilha.
