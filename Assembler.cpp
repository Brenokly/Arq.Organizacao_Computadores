...
int main() {
    ...
    if(!fin.is_open())
    {
        cout << "falha ao abrir o arquivo main.asm!" << endl;
    }
    ofstream fout;
    fout.open("assembler.hex");
    if(!fout.is_open())
    {
        cout << "falha ao abrir o arquivo assemble.hex!" << endl;
    }
    fout << "v2.0 raw\n";

    bitset<32> codigo;
	string ins{},r{};
    int endereco{0}, saida{1};
    ...
    while (!fin.eof())
    {
        ...
        for (int i = 0; i < 31; i++) {
            if (inst[i].name == ins) {
                codigo = inst[i].type(r, &inst[i], regis);
